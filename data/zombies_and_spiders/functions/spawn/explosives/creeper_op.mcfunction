...
execute if score count spawnCreeperOp matches 1 run summon creeper ~ ~ ~ {powered:1,Tags:["axolotvengance","not_op"],CustomName:'{"text":"Fulminant Drakxolot","color":"dark_purple"}',Passengers:[{CustomName:'{"text":"Fulminant Drakxolot","color":"dark_purple"}',id:axolotl}],Fuse:1, ExplosionRadius:10}
...
execute if score count spawnCreeperOp matches 1 run effect give @e[tag=axolotvengance] minecraft:poison 300 6
